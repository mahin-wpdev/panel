...
{if $d['coordinates']}
    ...
    {/literal}
{/if}
<div class="row"><div class="col-md-12"><div class="box box-info"><div class="box-header"><h3 class="box-title"><i class="fa fa-plug"></i> ONU / Fiber Network</h3></div><div class="box-body">{if $customerOnu}<table class="table table-bordered"><tr><th>OLT</th><th>PON / ONU</th><th>MAC</th><th>Status</th><th>RX / TX</th><th>Last Seen</th></tr><tr><td>{$customerOnu['olt_id']}</td><td>{$customerOnu['slot_number']}/{$customerOnu['pon_port']} / {$customerOnu['onu_id']}</td><td>{$customerOnu['mac_address']}</td><td>{$customerOnu['status']}</td><td>{$customerOnu['rx_power']} / {$customerOnu['tx_power']} dBm</td><td>{$customerOnu['last_seen']}</td></tr></table>{else}<form class="form-inline" method="post" action="{Text::url('customers/assign-onu/', $d['id'])}"><input type="hidden" name="csrf_token" value="{$csrf_token}"><label>Assign discovered ONU: </label> <select class="form-control" name="onu_id" required><option value="">Select ONU</option>{foreach $unassignedOnus as $onu}<option value="{$onu['id']}">{$onu['name']} — PON {$onu['slot_number']}/{$onu['pon_port']}, ONU {$onu['onu_id']} — {$onu['mac_address']}</option>{/foreach}</select> <button class="btn btn-primary">Assign ONU</button></form>{/if}</div></div></div></div>
<div class="row"><div class="col-md-12"><div class="box box-primary"><div class="box-header"><h3 class="box-title"><i class="fa fa-area-chart"></i> Traffic Monitor & Total Data Usage</h3><span class="pull-right text-muted">Auto refresh: 1 second</span></div><div class="box-body"><div id="traffic-state" class="text-muted">Connecting to MikroTik monitor…</div><div id="traffic-data" class="row" style="display:none"><div class="col-md-3"><b>Accounting Download</b><div id="total-download">—</div></div><div class="col-md-3"><b>Accounting Upload</b><div id="total-upload">—</div></div><div class="col-md-3"><b>Accounting Total</b><div id="total-usage">—</div></div><div class="col-md-3"><b>Live Status</b><div id="traffic-status">—</div></div><div class="col-md-3"><b>Current Download</b><div id="download-speed">—</div></div><div class="col-md-3"><b>Current Upload</b><div id="upload-speed">—</div></div><div class="col-md-3"><b>Interface Total Usage</b><div id="session-total">—</div></div><div class="col-md-3"><b>Connection Uptime</b><div id="session-uptime">—</div></div></div></div></div></div></div>
<div class="box box-default"><div class="box-header"><h3 class="box-title">Live Bandwidth Graph</h3></div><div class="box-body"><canvas id="traffic-chart" height="100"></canvas></div></div><script src="https://cdn.jsdelivr.net/npm/chart.js"></script><script>{literal}(function(){var chart;function byId(id){return document.getElementById(id);}function graph(x){if(!window.Chart)return;if(!chart){chart=new Chart(byId('traffic-chart').getContext('2d'),{type:'line',data:{labels:[],datasets:[{label:'Download Mbps',borderColor:'#00a65a',fill:false,data:[]},{label:'Upload Mbps',borderColor:'#00c0ef',fill:false,data:[]}]},options:{animation:false,responsive:true}});}var d=chart.data;d.labels.push(new Date().toLocaleTimeString());d.datasets[0].data.push((x.download_bps||0)/1000000);d.datasets[1].data.push((x.upload_bps||0)/1000000);if(d.labels.length>60){d.labels.shift();d.datasets[0].data.shift();d.datasets[1].data.shift();}chart.update();}function text(id,value){byId(id).textContent=value||'—';}function loadTraffic(){fetch('{/literal}{Text::url('customers/traffic/')}{$d['id']}{literal}',{credentials:'same-origin'}).then(function(r){if(!r.ok)throw new Error('request failed');return r.json();}).then(function(x){text('total-download',x.total_download);text('total-upload',x.total_upload);text('total-usage',x.total_usage);text('traffic-status',x.connected===true?'Online':(x.connected===false?'Offline':'Unavailable'));text('download-speed',x.download_speed);text('upload-speed',x.upload_speed);text('session-total',x.session_total);text('session-uptime',x.uptime);byId('traffic-data').style.display='block';graph(x);byId('traffic-state').textContent=x.accounting_available?'Live monitor + accounting total updated.':'Live monitor updated; accounting record unavailable.';}).catch(function(){byId('traffic-state').textContent='Traffic monitor is unavailable.';});}loadTraffic();setInterval(loadTraffic,1000);})();{/literal}</script>

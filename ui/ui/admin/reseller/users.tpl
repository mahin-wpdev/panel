...
<div class="panel panel-default panel-hovered panel-stacked mb30">
    ...
    <div class="panel-body table-responsive">
        <table class="table table-bordered table-hover"><thead><tr><th>Reseller</th><th>Agent username</th><th>Customers</th><th>Status</th><th></th></tr></thead><tbody>
        {foreach $resellers as $reseller}<tr><td>{$reseller['name']|escape}</td><td>{$reseller['username']|escape}</td><td>{$reseller['customer_count']}</td><td>{$reseller['status']|escape}</td><td class="text-right"><a class="btn btn-xs btn-primary" href="{Text::url('reseller/resellers/')}{$reseller['user_id']}"><i class="fa fa-pencil"></i> Configure</a></td></tr>{foreachelse}<tr><td colspan="5" class="text-center text-muted">No reseller profile exists yet.</td></tr>{/foreach}
        </tbody></table>
    </div>
</div>
{/if}

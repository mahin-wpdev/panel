...
function jm_mobile_resolve(PDO $db,string $username,string $password): ?array {
    ...
    $staff=jm_mobile_query($db,'SELECT id,username,fullname,password,user_type FROM tbl_users WHERE username=? LIMIT 1',[$username])->fetch(PDO::FETCH_ASSOC);
    $matches=[];
    ...
    if ($staff && hash_equals((string)$staff['password'],sha1($password))) {
        $reseller=jm_mobile_query($db,'SELECT id,status FROM tbl_resellers WHERE user_id=? LIMIT 1',[(int)$staff['id']])->fetch(PDO::FETCH_ASSOC) ?: null;

...
final class OltOnuRemoval
{
    ...
    private static function readUntil($socket, string $prompt, int $seconds = 12): string
    {
        ...
        while (microtime(true) < $deadline && strlen($output) < 131072) {
            ...
            if ($part !== '') $output .= $part;
            if (stripos($output, $pager) !== false) {
                $output = str_ireplace($pager, '', $output);
                if (fwrite($socket, ' ') === false) {
                    throw new RuntimeException('OLT pager continuation failed');
                }
            }
            // An intermediate pagination prompt cannot prove removal.
            if (preg_match('/'.preg_quote($prompt, '/').'\\s*$/i', $output)) {
                return $output;
            }
            usleep(50000);
        }
        throw new RuntimeException('OLT response incomplete or timed out; verify device manually');
    }
    ...
    public static function removeOffline($olt, $onu): void
    {
        ...
        try {
            ...
            $before = self::rows(self::send($socket, 'show olt 1 all-onu-info', self::PROMPT, 30));
            ...
            // Exact syntax remains untested on a retired ONU; require OLT verification.
            ...
            $after = self::rows(self::send($socket, 'show olt 1 all-onu-info', self::PROMPT, 30));
